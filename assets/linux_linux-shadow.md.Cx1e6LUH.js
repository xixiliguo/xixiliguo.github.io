import{_ as s,c as a,o as n,a2 as p}from"./chunks/framework.BufN96s7.js";const u=JSON.parse('{"title":"Linux /etc/shadow 文件学习笔记","description":"","frontmatter":{"title":"Linux /etc/shadow 文件学习笔记","author":"Peter Wang","tags":["linux","shadow","crypt"],"date":"2017-10-11T14:11:23.000Z","draft":false},"headers":[],"relativePath":"linux/linux-shadow.md","filePath":"linux/linux-shadow.md","lastUpdated":1721229891000}'),i={name:"linux/linux-shadow.md"},t=p(`<p>Linux系统下，创建的用户信息如ID，家目录，默认shell保存在/etc/passwd下，该文件每行的第二位（以冒号分隔）表示密码。但真正的密码其实被加密后放在/etc/shadow里，/etc/passwd里只显示为x。本文主要介绍shadow文件各字段含义和相关的密码生命周期配置。</p><p>本文内使用的Linux环境是centos7.3, 操作时间是2017年10月12号</p><h3 id="字段介绍" tabindex="-1">字段介绍 <a class="header-anchor" href="#字段介绍" aria-label="Permalink to &quot;字段介绍&quot;">​</a></h3><p>/etc/shadow每行和/etc/passwd一一对应，命令<code>pwconv</code>根据/etc/passwd生成。每行由9个字段（以冒号分隔）组成，如下是每个字段的含义：</p><h5 id="_1-登录名" tabindex="-1">1. 登录名: <a class="header-anchor" href="#_1-登录名" aria-label="Permalink to &quot;1. 登录名:&quot;">​</a></h5><p>如 root， 通过它，唯一匹配/etc/passwd中的一行</p><h5 id="_2-加密后的密码" tabindex="-1">2. 加密后的密码： <a class="header-anchor" href="#_2-加密后的密码" aria-label="Permalink to &quot;2. 加密后的密码：&quot;">​</a></h5><p>通常的格式为: $X$ZZZZZZ<br> X为数字，表示不同的加密算法，具体如下：</p><table tabindex="0"><thead><tr><th style="text-align:left;">ID</th><th style="text-align:left;">Method</th></tr></thead><tbody><tr><td style="text-align:left;">1</td><td style="text-align:left;">MD5</td></tr><tr><td style="text-align:left;">2a</td><td style="text-align:left;">Blowfish</td></tr><tr><td style="text-align:left;">5</td><td style="text-align:left;">SHA-256</td></tr><tr><td style="text-align:left;">6</td><td style="text-align:left;">SHA-512</td></tr></tbody></table><p>第二个星号后面的ZZZZ为加密后的密文, 具体是通过glibc里的crypt函数来实现加密。文章最后会提供一段程序，通过盐值来加密明文。可以通过<code>man 3 crypt</code>了解函数使用方法</p><p>如果该字段 是 ! 或者 *， 表示该用户无法用密码登录系统.但可以通过其他方式登录。<br> 如果该字段以感叹号!开头，其余是通常的格式，则系统认为密码被锁， ssh登录时即使输入正确的密码，也会拒绝登录. <code>usermod -L username</code> 就是使用该原理。<code>usermod -U username</code> 功能是解锁</p><h5 id="_3-密码最后修改时间" tabindex="-1">3. 密码最后修改时间： <a class="header-anchor" href="#_3-密码最后修改时间" aria-label="Permalink to &quot;3. 密码最后修改时间：&quot;">​</a></h5><p>通过<code>chage -d XX user</code>可以设置该字段<br> 值为从1970 1月1号至改密码时的天数。<br> 0表示用户需要在下次登录时修改密码<br> 空值表示关闭密码有效期功能，即密码永远有效<br> 如下是设置为0时的系统行为</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>[root@linux /root]# grep test /etc/shadow</span></span>
...
<span class="line"><span>$</span></span>
<span class="line"><span>然后用新密码就可以登录了， 这个可以用于管理员强制普通用户修改密码</span></span></code></pre></div><h5 id="_4-最小时间间隔" tabindex="-1">4. 最小时间间隔： <a class="header-anchor" href="#_4-最小时间间隔" aria-label="Permalink to &quot;4. 最小时间间隔：&quot;">​</a></h5><p>通过<code>chage -m XXX user</code>可以设置该字段<br> 两次修改口令之间所需的最小天数。<br> 空或者0表示没有限制</p><h5 id="_5-最大时间间隔" tabindex="-1">5. 最大时间间隔： <a class="header-anchor" href="#_5-最大时间间隔" aria-label="Permalink to &quot;5. 最大时间间隔：&quot;">​</a></h5><p>通过<code>chage -M XXX user</code>可以设置该字段<br> 两次修改口令之间所需的最大天数。一旦超过，意味着密码过期.<br> 空表示没有限制.</p><p>如果该值小于最小时间间隔，则用户无法修改密码,如下演示其行为:</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>[root@linux /root]# grep test /etc/shadow</span></span>
...
<span class="line"><span>Connection closed by 10.211.55.9</span></span></code></pre></div><h5 id="_8-用户过期时间" tabindex="-1">8. 用户过期时间： <a class="header-anchor" href="#_8-用户过期时间" aria-label="Permalink to &quot;8. 用户过期时间：&quot;">​</a></h5><p>通过<code>chage -E XXX user</code>可以设置该字段<br> 用户过期时间，值表示为自19701月1号起的天数.<br> 密码过期后，用户只是无法使用密码登陆，还可以用其他方式。 一旦用户过期，任何方式都无法用该用户登陆<br> 空值表示永远不会过期<br> 0值不建议使用。解释取决于程序本身</p><h5 id="_9-其他为保留字段-为将来扩展功能用" tabindex="-1">9. 其他为保留字段，为将来扩展功能用 <a class="header-anchor" href="#_9-其他为保留字段-为将来扩展功能用" aria-label="Permalink to &quot;9. 其他为保留字段，为将来扩展功能用&quot;">​</a></h5><h3 id="自己写代码实现加密-c-和-python实现" tabindex="-1">自己写代码实现加密(C 和 Python实现) <a class="header-anchor" href="#自己写代码实现加密-c-和-python实现" aria-label="Permalink to &quot;自己写代码实现加密(C 和 Python实现)&quot;">​</a></h3><div class="language-c vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">c</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;">#include</span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;"> &lt;crypt.h&gt;</span></span>
...
<span class="line"><span>&#39;$6$DEgVEU0T$mwlTGb/nTtvpIJcoIy2t9xNMgv0.IT34WLvQ7VmbWJP9rU8Ysp9JyJ8I8PxEleGPWoirdbk4VKbhtCg6P.sm1.&#39;</span></span></code></pre></div><h3 id="密码相关一些配置" tabindex="-1">密码相关一些配置 <a class="header-anchor" href="#密码相关一些配置" aria-label="Permalink to &quot;密码相关一些配置&quot;">​</a></h3><p>/etc/login.defs 用来存放一些与创建用户和密码相关的配置信息<br> 当使用useradd创建新用户时，系统会读取该文件，然后写入/etc/shadows.<br> 和密码相关的只要是 PASS_MAX_DAYS，PASS_MIN_DAYS，PASS_MIN_LEN，PASS_WARN_AGE，ENCRYPT_METHOD 这几个参数<br> UMASK用来定义默认的新建文件权限</p><div class="language- vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang"></span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>[root@linux /root]# grep ^[^#] /etc/login.defs</span></span>
